...
describe 'Authentication', () ->
  ...
  it 'should get an access token', (done) ->
    request api
      .get '/authentication/access?email=' + account.email + '&password=' + encodeURIComponent(account.password)
      .send()
      .end (e, res) ->
        res.status.should.equal 200
        res.body.success.should.equal true
  ...
  it 'should update an account', (done) ->
    request api
      .put '/authentication?token=' + account.request
      .send
        name: 'Harry'
  ...
  it 'should delete an account', (done) ->
    request api
      .delete '/authentication?token=' + account.request
      .send
        password: account.password
      .end (e, res) ->
        res.status.should.equal 200
